...
int _printf(const char *format, ...)
{
    ...
    while (*format)
    {
        if (*format == '%')
        {
            ...
            int flag_hash = 0;
            int flag_zero = 0;
            int flag_minus = 0; // Added for the minus flag
            ...
            switch (*format)
            {
                ...
                case 'i':
                    if (flag_plus && va_arg(args, int) >= 0)
                        ...
                        buffer[buffer_index++] = ' ';
                    if (flag_zero && precision == -1 && !flag_minus) // Only add leading zeros if zero flag is present and no precision is specified, and '-' flag is not present
                    {
                ...
                case 'x':
                    ...
                    if (flag_zero && precision == -1 && !flag_minus) // Add leading zeros for 'x' if zero flag is present and no precision specified, and '-' flag is not present
                    {
